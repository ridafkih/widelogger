...
import { getAllAdapters, registerAdapter } from "./platforms";
...
export const setup = (({ env }) => {
  return widelog.context(() => {
    ...
    const config = {
      ...
      imessageWatchedContacts:
        env.IMESSAGE_WATCHED_CONTACTS?.split(",").filter(Boolean) ?? [],
      imessageContextMessages: Number.parseInt(
        env.IMESSAGE_CONTEXT_MESSAGES ?? "20",
        10
      ),
      staleSessionThresholdMs: Number.parseInt(
        env.STALE_SESSION_THRESHOLD_MS ?? "86400000",
        10
      ),
    };
